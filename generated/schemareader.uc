// Automatically generated from ./urender.schema.pretty.json - do not edit!
"use strict";
...
function instantiateSwitch(location, value, errors) {
	try {
		let module = require('schemaSwitch');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-switch."  ]);
	}
}
...
function instantiateInterface(location, value, errors) {
	if (type(value) == "object") {
		...
		function parseSelectUpstream(location, value, errors) {
			if (type(value) != "string")
				push(errors, [ location, "must be of type string" ]);

			if (!(value in [ "match-vlan", "default", "none" ]))
				push(errors, [ location, "must be one of \"match-vlan\", \"default\" or \"none\"" ]);

			return value;
		}

		if (exists(value, "select-upstream")) {
			obj.select_upstream = parseSelectUpstream(location + "/select-upstream", value["select-upstream"], errors);
		}
		else {
			obj.select_upstream = "match-vlan";
		}
...
function instantiateServiceLldp(location, value, errors) {
	try {
		let module = require('schemaServiceLldp');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-lldp."  ]);
	}
}
...
function instantiateServiceMdns(location, value, errors) {
	try {
		let module = require('schemaServiceMdns');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-mdns."  ]);
	}
}
...
function instantiateServiceIgmp(location, value, errors) {
	try {
		let module = require('schemaServiceIgmp');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-igmp."  ]);
	}
}

function instantiateServiceIeee8021x(location, value, errors) {
	try {
		let module = require('schemaServiceIeee8021x');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-ieee8021x."  ]);
	}
}
...
function instantiateServiceDataPlane(location, value, errors) {
	try {
		let module = require('schemaServiceDataPlane');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-dataplane."  ]);
	}
}

function instantiateServiceWifiSteering(location, value, errors) {
	try {
		let module = require('schemaServiceWifiSteering');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-usteer."  ]);
	}
}
...
function instantiateServiceQualityOfServiceClassSelector(location, value, errors) {
	...
	if (!(value in [ "besteffort", "bulk", "video", "voice", "CS0", "CS1", "CS2", "CS3", "CS4", "CS5", "CS6", "CS7", "AF11", "AF12", "AF13", "AF21", "AF22", "AF23", "AF31", "AF32", "AF33", "AF41", "AF42", "AF43", "DF", "EF", "VA", "LE" ]))
		push(errors, [ location, "must be one of \"besteffort\", \"bulk\", \"video\", \"voice\", \"CS0\", \"CS1\", \"CS2\", \"CS3\", \"CS4\", \"CS5\", \"CS6\", \"CS7\", \"AF11\", \"AF12\", \"AF13\", \"AF21\", \"AF22\", \"AF23\", \"AF31\", \"AF32\", \"AF33\", \"AF41\", \"AF42\", \"AF43\", \"DF\", \"EF\", \"VA\" or \"LE\"" ]);
...
function instantiateServiceQualityOfService(location, value, errors) {
	try {
		let module = require('schemaServiceQualityOfService');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-qos."  ]);
	}
}
...
function instantiateServiceGps(location, value, errors) {
	try {
		let module = require('schemaServiceGps');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-gps."  ]);
	}
}

function instantiateServiceUnet(location, value, errors) {
	try {
		let module = require('schemaServiceUnet');
		return module.validate(location, value, errors);
	} catch(e) {
		push(errors, [ location, "is missing its module. Please install urender-mod-unet."  ]);
	}
}
...
function instantiateService(location, value, errors) {
	if (type(value) == "object") {
		...
		if (exists(value, "unet")) {
			obj.unet = instantiateServiceUnet(location + "/unet", value["unet"], errors);
		}
