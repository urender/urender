...
{%   if (interface.select_upstream in [ 'match-vlan', 'default' ]): %}
add firewall forwarding
set firewall.@forwarding[-1].src={{ s(name) }}
set firewall.@forwarding[-1].dest='{{ s(dest || ethernet.find_interface('upstream', interface.select_upstream == 'match-vlan' ? interface.vlan.id : 0)) }}'
{%   endif %}
{% endif %}
