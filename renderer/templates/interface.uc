{%
	...
	if (ethernet.has_vlan(interface) && interface.role == 'downstream' &&
	    interface.select_upstream == "match-vlan" && index(vlans_upstream, this_vid) < 0) {
	...
	/* setup broadand uplink */
	if (interface.broad_band) {
		/* broadband uplink is only allowed on 'upstream' interfaces */
		if (interface.role != 'upstream') {
			warn('Only upstream interfaces can have broadband settings');
